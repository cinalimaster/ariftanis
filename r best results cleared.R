...
setwd("~/R ÇALIÞMALARIM/r working dir")


## kolonlara daðýlmýþ Fon adlarýnýn skorlarýný tek kolonda topluyoruz
testdb2 <-gather(testdb,no,"score",4:75)

## bazý kolonlarý çýkardýk
testdb2a <- testdb2[,-3]
testdb2c <- testdb2[-(6233:6440),]

## tarih olmayan satýrlarý çýkarýyoruz
...
?write.csv
## sonuçlarýmýzý csv olarak kaydettik
write.csv(testdb3, file = "E:/okul/_PHD 5-2gb/okul tez/BIG DATA/hesaplama I/testdb.csv")


db <- testdb2y001

## satýr numarasý olmayan matrise ID adýnda bir sütun oluþturup satýr numaralarý verdik
testdb3$ID <- seq.int(nrow(testdb3))
y001 <- testdb3$x001

## y001 deðiþkenini data frame haline getirdik
y001 <- as.data.frame(y001)
y001 <- y001[-1,]
str(db)

## convert to numeric bazý veriler factor veya string olarak tanýmlanmýþ bunlarý numeric'e dönüþtürüyoruz.
db[, c(4:66)] <- sapply(db[, c(4:66)], as.numeric)
str(db)


## veri olmayan eksik verisi olan satýrlarý çýkardýk.
db2 <- na.omit(db)
db3 <- db
db <- db2
...
## tüm deðiþken adlarýný bir deðiþken yaptý
allVars <- colnames(db)  

## 5. kolon ile 66. kolon isimlerini tek bir deðiþken olarak atadýk.
predictorVars <- colnames(db[,5:66])

## X date no y001 x001 hariç tüm deðiþken adlarýný bir deðiþken yaptý (bu kodu kullanmadým)
predictorVars <- allVars[!allVars %in% c("X","date","no","y001","x001")] 

## tüm predictor variable isimlerini arasýna + yazdýrdý
predictorVars <- paste(predictorVars, collapse ="+") 

## neuralnet de kullanacaðýmýz formülü tanýmladýk.
form=as.formula(paste("y001~", predictorVars, collapse = "+"))

# Data Partition data mýzý %30 %70 olacak þekilde rastgele böldük test ve train parçalarýný oluþturduk
set.seed(222)
...
# Prediction burada neuralnet ile tespit ettiðimiz çarpan ve sabitleri kullanarak modeli çalýþtýrýp
# bilinen deðiþkenleri girdi olarak verdiðimizde elde edeceðimiz tahmin'i oluþturup orjinal veri ile karþýlaþtýrýyoruz.
...
# net.result db sinin içindeki ilk 5 sütun olan "V1,X,date,no,y001,x001" sütunlarýndan oluþan yeni bir dataframe tanýmladýk.
totalmerged <- merge(output$net.result,db[,1:5])
View(totalmerged)

# sonuçlarýmýzý kaydettik
...
# X sütunundaki bilgileri esas alarak resultDB ve db dataframelerini birleþtirip total adýnda yeni bir dataframe oluþturduk.
...
## neuralnet de kullanacaðýmýz formülü tanýmladýk. y001 for =========================================================RETURN BASED
form=as.formula(paste("y001~", predictorVars, collapse = "+"))
...
# sonuçlarýmýzý kaydettik
write.csv(totalmerged, file = "~/R ÇALIÞMALARIM/r working dir/returnbased_Results2-20-10-5.csv")
write.csv(db, file = "C:/R/db.csv")
...
## neuralnet de kullanacaðýmýz formülü tanýmladýk. y001 for =========================================================RISK BASED
form=as.formula(paste("y001~", predictorVars, collapse = "+"))
...
# sonuçlarýmýzý kaydettik
write.csv(totalmerged, file = "~/R ÇALIÞMALARIM/r working dir/riskbased_Results-20-10-5.csv")
write.csv(db, file = "C:/R/db.csv")
